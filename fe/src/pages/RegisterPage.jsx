...
import { FiUser, FiMail, FiLock, FiUserPlus, FiShoppingBag, FiEye, FiEyeOff, FiCheck } from "react-icons/fi";
...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-navy via-navy-light to-slate">
      ...
      <div className="absolute bottom-1/4 -left-20 w-96 h-96 bg-primary-light/15 rounded-full blur-3xl" />
      <div className="absolute bottom-10 right-1/4 w-40 h-40 bg-accent/10 rounded-full blur-2xl" />

      <div className="relative z-10 w-full max-w-md mx-4 py-8 animate-fade-in">
        {/* Brand */}
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl shadow-navy/50 p-8 sm:p-10">
          <div className="text-center mb-8">
            ...
            <p className="text-white/60 text-sm">Daftar untuk mulai belanja di ShopWish</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            {/* Name */}
            <div>
              <label className="block text-white/70 text-xs font-semibold mb-2 uppercase tracking-wider">Nama Lengkap</label>
            ...
            <div>
              ...
              {password.length > 0 && (
                <div className="flex items-center gap-1.5 mt-2">
                  <div className={`h-1 flex-1 rounded-full transition-colors ${password.length >= 6 ? "bg-success" : "bg-accent/50"}`} />
                  <div className={`h-1 flex-1 rounded-full transition-colors ${password.length >= 8 ? "bg-success" : "bg-white/10"}`} />
                  <div className={`h-1 flex-1 rounded-full transition-colors ${password.length >= 10 ? "bg-success" : "bg-white/10"}`} />
                  <span className="text-[10px] text-white/40 ml-1">
                    {password.length < 6 ? "Lemah" : password.length < 8 ? "Cukup" : password.length < 10 ? "Kuat" : "Sangat Kuat"}
                  </span>
                </div>
              )}
            </div>
            ...
            {/* Confirm Password */}
            <div>
              <label className="block text-white/70 text-xs font-semibold mb-2 uppercase tracking-wider">Konfirmasi Password</label>
              <div className="relative">
                <FiCheck className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40" />
                <input
                  type={showPassword ? "text" : "password"}
                  ...
                  required
                  className={`w-full pl-11 pr-4 py-3.5 rounded-xl bg-white/10 border text-white placeholder-white/30 text-sm focus:outline-none focus:ring-2 transition-all ${
                    confirmPassword && confirmPassword !== password
                      ? "border-accent/50 focus:ring-accent/50"
                      : "border-white/15 focus:ring-primary/50 focus:border-primary/50"
                  }`}
                />
              </div>
            </div>
          ...
          <div className="flex items-center gap-4 my-7">
            ...
            <span className="text-white/40 text-xs">atau daftar dengan</span>
            <div className="flex-1 h-px bg-white/15" />
          </div>
